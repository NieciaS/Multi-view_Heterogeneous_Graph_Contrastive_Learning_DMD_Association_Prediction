...
def constrative_loss(self, output1, output2, target):
...
def main(args):
    ...
    # Load disease and microbes data file (FCNN model)
    smiles_file = '/content/drive/MyDrive/HTN-main_Code/HTN-main/drug_smiles_270.csv'
    
    batch_drug = drug_fea_process(smiles_file, drug_num=270)
    print("SMiles file /" , batch_drug)
    
    
    dis_sim = np.loadtxt('/content/drive/MyDrive/HTN-main_Code/HTN-main/dis_sim.txt', delimiter='\t')
    mic_sim = np.loadtxt('/content/drive/MyDrive/HTN-main_Code/HTN-main/mic_sim_NinimHMDA.txt', delimiter='\t')
    ...
    model_MCHNN = BioEncoder(mic_sim.shape[0], dis_sim.shape[0], 32,[32, 8, 2])
    
    # Define loss function and optimizer
    #criterion = nn.BCEWithLogitsLoss()
    #optimizer = torch.optim.Adam(model_HTN.parameters(), lr=0.005) + torch.optim.Adam(model_MCHNN.parameters(), lr=0.005)
    model_HTN_params = list(model_HTN.parameters())
    model_MCHNN_params = list(model_MCHNN.parameters())
    all_params = model_HTN_params + model_MCHNN_params  # Combine parameter groups
    ...
    for epoch in range(args["num_of_epochs"]):
      ...
      for batch in train_loader:
          ...
          print(f"BioEncoder drug_embed_bio shape: {x_dru.shape}")
          print (f"BioEncoder mic_embed_bio shape: {x_mic.shape}")
          print(f"BioEncoder dis_embed_bio shape: {x_dis.shape}")
          print(f"HTN output drug_embed_htn shape: {out_HTN.shape}")
          ...
          y_pred_MCHNN = model_MCHNN.forward_predictor(out_MCHNN, drug_microbe_disease_batch[:, 0], drug_microbe_disease_batch[:, 1], drug_microbe_disease_batch[:, 2])
          #print(y_pred)

          # Calculate loss
          loss = constrative_loss(y_pred, y_pred_MCHNN, y_batch)
    ...
    torch.save(model_HTN.state_dict(), '/content/drive/MyDrive/HTN-main_Code/model_HTN.pth')
    torch.save(model_MCHNN.state_dict(), '/content/drive/MyDrive/HTN-main_Code/model_MCHNN.pth')
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            y_pred_MCHNN = model_MCHNN.forward_predictor(out_MCHNN_test, drug_microbe_disease_batch[:, 0], drug_microbe_disease_batch[:, 1], drug_microbe_disease_batch[:, 2])
