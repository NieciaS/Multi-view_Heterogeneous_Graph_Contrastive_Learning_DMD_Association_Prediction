...
def main(args):
    ...
    model_MCHNN = BioEncoder(mic_sim.shape[0], dis_sim.shape[0], 32)
    ...
    for epoch in range(args["num_of_epochs"]):
      ...
      for batch in train_loader:
          ...
          x_dru, x_mic, x_dis = model_MCHNN(batch_drug, x_mic, x_dis)
          out_MCHNN = torch.cat((x_dru, x_mic, x_dis), 0)

          # Get the predicted interaction values
          y_pred = model_HTN.forward_predictor(out, drug_microbe_disease_batch[:, 0], drug_microbe_disease_batch[:, 1], drug_microbe_disease_batch[:, 2])
          y_pred_MCHNN= model_MCHNN.forward_predictor()
